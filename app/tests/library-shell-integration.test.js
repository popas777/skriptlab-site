...
test('Library is the last module after corrections and is mounted in an accessible iframe', () => {
  ...
  assert.match(
    indexHtml,
    /id=["']view-kirjasto["'][^>]*class=["'][^"']*view-section[^"']*hidden[^"']*["'][\s\S]*?<iframe\b(?=[^>]*id=["']kirjasto-frame["'])(?=[^>]*class=["']kirjasto-frame["'])(?=[^>]*src=["']kirjasto\.html\?v=6["'])(?=[^>]*title=["']Julkaistujen teosten kirjasto["'])/
  );
...
test('Shell assets use the current cache versions', () => {
  ...
  assert.equal(numericAssetVersion(indexHtml, 'kirjasto.html'), 6);
});
